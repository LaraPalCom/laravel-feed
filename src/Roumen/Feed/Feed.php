...
class Feed
{
     ...
     * @param \DateTimeInterface|int|null $cache  (0 - turns off the cache)
    ...
    public function render($format = null, $cache = null, $key = null)
    {
        ...
        if ($this->caching > 0 && Cache::has($this->cacheKey)) {
            return Response::make(Cache::get($this->cacheKey), 200, ['Content-Type' => $this->ctype.'; charset='.$this->charset]);
        }
        ...
        if ($this->caching > 0) {
            ...
            return Response::make(View::make($this->getView($this->customView), $viewData), 200, ['Content-Type' => $this->ctype.'; charset='.$this->charset]);
        } elseif ($this->caching < 0) {
    ...
    /**
     * Set maximum characters length for text shortening
     *
     * @param int $l
     *
     * @return void
